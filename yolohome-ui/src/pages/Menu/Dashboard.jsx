   ...
   return (
      <section className="uc1-page app-dark-shell">
         ...
         <div className="uc-page-head">
            <div>
               <span className="uc-page-head__eyebrow">
                  UC 1 · Giám sát môi trường thời gian thực
               </span>
               ...
               <div className="uc1-hero glass-panel glass-panel--inner">
                  ...
                  <div className="uc1-hero__actions">
                     {scenario.controlActions.map((action, index) => {
                        const Icon =
                           index === scenario.controlActions.length - 1
                              ? RefreshCw
                              : action.includes('Lịch')
                                ? History
                                : action.includes('Trang')
                                  ? House
                                  : Gauge
                        return (
                           <button
                              key={action}
                              type="button"
                              className={`ghost-pill ${index === 1 ? 'is-dark' : ''}`}
                           >
                              <Icon size={16} />
                              <span>{action}</span>
                           </button>
               ...
               <div className="uc1-current glass-panel glass-panel--inner">
                  <div className="section-head">
                     <div>
                        ...
                        <h3>
                           {activeTab === 'reload' ? 'Dữ liệu sau khi tải lại' : 'Dữ liệu hiện tại'}
                        </h3>
                     </div>
                     <div className="section-head__actions">
                        <button
                           type="button"
                           className={`mini-circle ${scenario.unit === 'C' ? 'is-dark' : ''}`}
                        >
                           °C
                        </button>
                        <button
                           type="button"
                           className={`mini-circle ${scenario.unit === 'F' ? 'is-dark' : ''}`}
                        >
                           °F
                        </button>
                  ...
                  <div
                     className={`sensor-grid ${scenario.cards.length <= 2 ? 'sensor-grid--compact' : ''}`}
                  >
                     {scenario.cards.map((card) => {
                        ...
                        return (
                           <article
                              key={card.label}
                              className={`sensor-card ${card.state ? `is-${card.state}` : ''}`}
                           >
                              ...
                              <div className="inline-actions">
                                 {scenario.supportPanel.controls.map((control, index) => (
                                    <button
                                       key={control}
                                       type="button"
                                       className={`mini-circle ${index === 0 ? 'is-dark' : ''}`}
                                    >
                                       {control}
                                    </button>
                                 ))}
                     ...
                     <div className="side-card glass-panel glass-panel--inner">
                        <div className="section-head section-head--compact">
                           <div>
                              ...
                              <h3>
                                 {scenario.supportPanel.title === 'Lịch sử dữ liệu'
                                    ? 'Lịch sử dữ liệu'
                                    : 'Biểu đồ gần nhất'}
                              </h3>
                           </div>
                        </div>
                        {scenario.supportPanel.subtitle && (
                           <p className="support-copy">{scenario.supportPanel.subtitle}</p>
                        )}
                  ...
                  <div className="side-card glass-panel glass-panel--inner">
                     ...
                     <div className="status-list status-list--stacked">
                        {scenario.supportPanel.statusRows.map((row) => (
                           <div
                              key={row.label}
                              className={`status-pill ${row.tone ? `is-${row.tone}` : ''}`}
                           >
                              <span>{row.label}</span>
                              <strong>{row.value}</strong>
                           </div>
