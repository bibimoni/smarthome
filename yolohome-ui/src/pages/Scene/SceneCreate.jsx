   ...
   const handleSave = async () => {
      ...
      try {
         if (isEditMode) {
            ...
            const actionKey = (item) => `${item.actuator_id}::${item.action_value}`
            const conditionKey = (item) => `${item.sensor_id}::${item.operator}::${Number(item.threshold_value)}`

            const deletedActIds = initialActions
               .filter((initialItem) => {
                  const currentItem = actions.find((item) => item.id === initialItem.id)
                  return !currentItem || actionKey(currentItem) !== actionKey(initialItem)
               })
               .map((item) => item.id)

            const changedOrAddedActs = actions.filter((item) => {
               const initialItem = initialActions.find((initialValue) => initialValue.id === item.id)
               return !initialItem || actionKey(initialItem) !== actionKey(item)
            })

            const deletedCondIds = initialConditions
               .filter((initialItem) => {
                  const currentItem = conditions.find((item) => item.id === initialItem.id)
                  return !currentItem || conditionKey(currentItem) !== conditionKey(initialItem)
               })
               .map((item) => item.id)

            const changedOrAddedConds = conditions.filter((item) => {
               const initialItem = initialConditions.find((initialValue) => initialValue.id === item.id)
               return !initialItem || conditionKey(initialItem) !== conditionKey(item)
            })
            ...
            changedOrAddedActs.forEach((item) => {
               promises.push(
                  addAction(editId, {
            ...
            changedOrAddedConds.forEach((item) => {
               promises.push(
                  addCondition(editId, {
                     sensor_id: item.sensor_id,
                     operator: item.operator,
                     threshold_value: parseFloat(item.threshold_value),
                  }).catch(console.error),
               )
            })
